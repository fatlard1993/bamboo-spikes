...
public class BambooSpikesBlock extends Block implements Waterloggable {
	...
	public BlockState getStateForNeighborUpdate(BlockState state, Direction facing, BlockState neighborState, IWorld world, BlockPos pos, BlockPos neighborPos){
		if((Boolean)state.get(WATERLOGGED)) world.getFluidTickScheduler().schedule(pos, Fluids.WATER, Fluids.WATER.getTickRate(world));
	...
	public BlockState getPlacementState(ItemPlacementContext context){
		...
		return (BlockState)((BlockState)this.getDefaultState().with(FACING, blockState.getBlock() == this && blockState.get(FACING) == direction ? direction.getOpposite() : direction)).with(WATERLOGGED, waterlogged);
	}
